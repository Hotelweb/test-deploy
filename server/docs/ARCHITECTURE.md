...
### Phase 1 MVP guest journey

Luồng chính hiện được hỗ trợ:

1. Guest mở `/hotel/:slug` từ QR/token khách sạn. Backend lookup qua `GET /hotels/slug/:slug` hoặc `GET /hotels/qr/:qrToken`; nếu `ANALYTICS_ENABLED=true`, mỗi lần mở được ghi vào `hotel_qr_open_events` với trường `source` và `room_token` dự phòng cho QR theo phòng.
2. Trang guest hiển thị thông tin khách sạn, danh sách dịch vụ đang bật và chi tiết dịch vụ. Dịch vụ `food_order` điều hướng sang `/hotel/:slug/order/:serviceId`.
3. Guest đặt F&B qua `POST /food-order/orders`. Đơn yêu cầu ít nhất `room_number` hoặc `customer_phone`, snapshot tên món/category/giá/số lượng, sinh `order_code`, và hỗ trợ `idempotency_key`.
4. Hotel admin xử lý đơn trong `/admin/:hotelId/food-order`. Trạng thái hợp lệ: `new -> accepted -> preparing -> delivering -> completed`; từ các bước đang xử lý có thể `cancelled`, và từ `new` có thể `rejected` kèm lý do.
5. Socket.IO namespace `/chat` phát realtime `orderCreated`, `orderStatusChanged`, `newMessage`, `sessionUpdate` cho admin/guest room tương ứng.
6. Chat guest/admin lưu session và message history theo `hotel_id`; staff endpoints và socket staff events đều kiểm tra hotel access bằng JWT scope `system`/`hotel`.
...
- **Đơn:** `food_orders.status` dùng lifecycle `new → accepted → preparing → delivering → completed`, có nhánh `rejected`/`cancelled`; snapshot dòng đơn trong `food_order_items` (tên + giá tại thời điểm đặt).
...
| `food_orders.status` | new, accepted, preparing, delivering, completed, cancelled, rejected |
