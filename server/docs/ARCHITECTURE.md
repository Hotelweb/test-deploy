...
Migration SQL cũ đã được gom vào `src/database/seeds/seed.ts`. Với local dev, seed là nguồn schema chính:

| Cơ chế | Lệnh | Vai trò |
|--------|------|---------|
| **Seed SQL** | `pnpm run seed` | Tạo/cập nhật schema + dữ liệu mẫu (idempotent). Bao gồm các thay đổi từng nằm trong migration 001-005. |
